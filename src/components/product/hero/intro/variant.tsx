...
export default function Variant() {
  ...
  return (
    <div className="space-y-2 mt-2">
      {options.map((ele) => (
        <div key={ele.name}>
          <h4 className="font-area font-semibold text-[12px] sm:text-[0.875rem] text-black/50 uppercase mb-1.5">
            {ele.name}
          </h4>
          ...
          >
            <SelectTrigger className="w-full  font-area border-1 !py-5 px-4 border-black rounded-full font-semibold text-[1.125rem] !h-[45px]">
              <SelectValue />
            </SelectTrigger>
            ...
            <SelectContent className="max-h-56 rounded-2xl border-black">
              {ele.values.map((option) => (
                <SelectItem
                  key={option}
                  className="font-semibold leading-0 text-[1.125rem] font-area"
                  value={option}
                >
                  {option}
                </SelectItem>
